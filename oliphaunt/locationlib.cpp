...
float calculateMidpoint() {
    float min_angle=fabs(PAN_BUFFER.angle[0]), temp, midpoint;
    for (int i =0; i<PAN_BUFFER.index+1; i++) {
        temp = fabs(PAN_BUFFER.angle[i]);
        if (temp < min_angle) {
            midpoint=i;
            min_angle = temp;            
        }
...
posn_vect triangulation() {
    ...
    for (int i=0; midpoint+i<PAN_BUFFER.index+1 || midpoint-i>0; i++) {
        // Add central point (where angle == 0)
        if (i==0) {
            polar2coor(PAN_BUFFER.dist[midpoint], PAN_BUFFER.angle[midpoint], &x_temp, &y_temp);
        ...
        if (midpoint+i < num_readings) {
            //fout<<midpoint+i;
            polar2coor(PAN_BUFFER.angle[midpoint+i], PAN_BUFFER.angle[midpoint+i], &x_temp, &y_temp);
